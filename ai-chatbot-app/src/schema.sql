...
create table sessions (
  id uuid primary key default gen_random_uuid(), -- 세션 고유 ID (UUID)
  user_id uuid references users(id) not null, -- 어떤 사용자의 세션인지
...
create table conversations (
  ...
  user_id uuid references users(id) not null, -- 대화방 소유자
  created_at timestamp with time zone default timezone('utc'::text, now()) not null -- 생성 시간
);
...
create table messages (
 ...
 user_id uuid references users(id), -- 메시지 작성자 (시스템 메시지는 null)
